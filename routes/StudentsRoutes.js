...
router.post('/create', [
    ...
    check('Name_direction', "Направление студента не может быть пустым").notEmpty()
], authMiddleware, roleMiddleware([1,3]), controller.create);
...
router.put('/:id', [
    ...
    check('Name_direction').optional().notEmpty().withMessage('Направление студента не может быть пустым'),
], authMiddleware, roleMiddleware([3]), controller.updateStudent);
router.get('/thisYear', authMiddleware, roleMiddleware([1,3]), controller.getStudentsDefThisYear)
